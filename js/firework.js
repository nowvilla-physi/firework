...
class FireWork {
    ...
    rising() {
        ...
        this.x += this.vx;
        this.y -=
            this.vy * ((this.fireHeight - (height - this.y)) / this.fireHeight);
    ...
    explosion() {
        for (let explosion of this.explosions) {
            explosion.frame++;
            // 爆発し終わった花火を配列から除去する
            if (explosion.getType === 2) {
                this.explosions = this.explosions.filter((n) => n !== explosion);
                continue;
            }

            // 残像を描画
            if (Math.round(random(0, 32)) === 0) {
                explosion.afterImages.push(
                    new Afterimage(
                        this.r,
                        this.g,
                        this.b,
            ...
            for (let ai of explosion.afterImages) {
                if (ai.getAlpha < 0) {
                    explosion.afterImages = explosion.afterImages.filter((n) => n !== ai);
                    continue;
                }
                ai.exImage();
            }
            ...
            // 爆発を描画
            this.update(explosion.x, explosion.y, explosion.w, explosion.a);
            explosion.x += explosion.vx;
            explosion.y += explosion.vy;
            explosion.vx = explosion.vx * explosion.gv;
            explosion.vy = explosion.vy * explosion.gv;
            explosion.vy = explosion.vy + explosion.gv / 30;
            if (this.exEnd < explosion.frame) {
                explosion.w -= 0.1;
                explosion.a = explosion.a - 4;
                if (explosion.a < 0 && explosion.afterImages.length === 0) {
                    explosion.type = 2;
                }
            }
        }
